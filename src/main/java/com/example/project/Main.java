...
public class Main{
    ...
    */
    public static ArrayList<String> insertAfterI(ArrayList<String> stringList, String str){
       ArrayList<String> newStringList = stringList;
       for (int i = 0; i < stringList.size(); i++) {
        if (stringList.get(i).indexOf("i") != -1) {
            newStringList.add(i + 1, str);
            i++;
        }
       }
   ...
   */
    ...
    public static ArrayList<Integer> notAlone(ArrayList<Integer> list, int val){
        for (int i = 1;  i < list.size() - 1; i++) {
            boolean check = list.get(i) == val && list.get(i-1) != list.get(i) && list.get(i+1) != list.get(i);
            if (check) {
                list.set(i, Math.max(list.get(i-1), list.get(i+1))); 
            } 
        }
        boolean checkFirst = list.get(0) == val && list.get(1) > list.get(0);
        if (checkFirst) {
            list.set(0, list.get(1));
        }
        boolean checkLast = list.get(list.size() - 1) == val && list.get(list.size() - 2) > list.get(list.size() - 1);
        if (checkLast) {
    ...
    public static ArrayList<Integer> fix34(ArrayList<Integer> list){
        ...
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i) == 3 && (i == list.size() - 1 || list.get(i + 1) != 4)) {
                ...
                list.set(i + 1, 4);
                list.set(four.get(fourIndex), temp);
                fourIndex++;
            }
        }
        return list;
